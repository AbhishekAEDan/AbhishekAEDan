<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=28&pause=1000&color=00E5A0&center=true&vCenter=true&width=700&lines=Hi%2C+I'm+Abhishek+Dan;Systems+Administrator+%26+Developer;I+build+things+that+run+in+production." alt="Abhishek Dan" />

**CS graduate from UWI · Trinidad & Tobago**
Live infrastructure, real software, and systems that don't go down.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-abhishekdan1-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abhishekdan1/)
[![Email](https://img.shields.io/badge/Email-danabhishek.work%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:danabhishek.work@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-CV_Site-00E5A0?style=for-the-badge&logo=cloudflarepages&logoColor=black)](https://github.com/AbhishekAEDan/cv-site)

<br/>

| 🖥️ **26** containers in production | ⏱️ **99.97%** uptime · 90 days | 📱 **1** app on Google Play |
|:---:|:---:|:---:|

</div>

---

## 🧭 About Me

```text
abhishek@eshwarhq:~$ cat about.txt
```

I'm a Computer Science graduate who works at every layer of the stack: from **Cisco VLAN cutovers** and **Docker Compose orchestration** to **React + Supabase** apps and **multi-agent AI pipelines**.

- 🐧 Run a **26-container Ubuntu 24.04 LTS** production environment behind **Cloudflare Zero Trust**, with nightly Restic backups and custom observability.
- 🤖 Design **multi-agent Claude Code workflows**: an orchestrator dispatching tiered subagents with escalation rules and manager-side verification.
- 🌐 Diagnosed and fixed real incidents in live enterprise networks (UniFi, Cisco Catalyst, Microsoft 365, multi-WAN failover).
- 🎓 BSc Computer Science, **University of the West Indies**, St. Augustine · Google Cybersecurity & Google Project Management Professional Certificates.
- 🔍 Open to roles in **systems administration, DevOps, backend, or AI-workflow engineering**.

---

## 🚀 Featured Work

| Project | What it is | Stack |
|---|---|---|
| **[Gasparillo Hindu School](https://gasparillohinduschool.com/)** ★ Live | End-to-end school management platform: student records, enrollment, term marks, SEA results, PDF reports, news CMS. 14-table Postgres schema with RLS on every table. | React · Vite · Supabase · Cloudflare Pages · R2 |
| **[TriniPoster](https://github.com/AbhishekAEDan/triniposter-privacy-policy)** 📱 Play Store | Trinidad-localised social deduction party game (3–10 players), v1.3.0. Remote announcements, RevenueCat + AdMob monetisation, full EAS release pipeline. | React Native · Expo · TypeScript · Supabase |
| **[Gist](https://github.com/AbhishekAEDan/Gist-releases)** 🎙️ Android | On-device AI meeting recorder: timestamped transcripts, speaker labels, and AI-generated summaries, decisions and action items. No backend, no account. | Kotlin · Jetpack Compose · AssemblyAI · Claude |
| **[The Oracle](https://github.com/AbhishekAEDan/TheOracleDND-releases)** 🎲 D&D | Discord bot that transcribes live D&D sessions with faster-whisper and posts AI narrative summaries every 10 minutes via local Ollama. Fully offline. | Python · discord.py · faster-whisper · Ollama |
| **Live Production Infrastructure** 🖥️ | 26-service Linux stack delivering media, storage, document management, and automation. Zero-downtime change discipline, automated backups, Zero Trust access. | Ubuntu · Docker · Cloudflare ZT · Tailscale · Restic |
| **SpotTheLiar** 🔬 | Read-only MelonLoader mod for Liar's Bar (Unity IL2CPP). Recovered source via ILSpy, ported to Il2CppInterop, verified the server-authoritative trust model. | C# · .NET 6 · MelonLoader · Il2CppInterop |
| **[Token Flip](https://github.com/AbhishekAEDan/token-flip-clone)** 🎭 Foundry VTT | System-agnostic module letting tokens switch between saved faces mid-session, with HP sharing and a public API. | JavaScript · Foundry VTT API |
| **[Eshwar](https://eshwarhq.com/)** 🎵 Live | Official artist site: cinematic hero, mouse-tracked spotlight, streaming embeds. Zero build step. | React (CDN) · HTML/CSS · Formspree |

<details>
<summary><b>More projects</b></summary>
<br/>

- **[Cards Game](https://github.com/AbhishekAEDan/cards-game)** · Real-time multiplayer card game platform · JavaScript
- **[Discord Manager Bot](https://github.com/AbhishekAEDan/Discord-Manager-Bot---Pro-Heroes-Association-for-Plus-Ultra-Server)** · Modular moderation bot with scam-link detection and chat leveling · Python, Pycord
- **[Weathery](https://github.com/AbhishekAEDan/Weathery)** · Liquid-glass Flask weather app on OpenWeatherMap · Python, Flask
- **[Island Adventure](https://github.com/AbhishekAEDan/Island-Adventure)** · 2D RPG on a custom Java game engine: sprite animation, hitbox collision, NPC AI, inventory · Java
- **[HotSpott](https://github.com/AbhishekAEDan/HotSpott-Website)** · Current events discovery for Trinidad & Tobago · HTML, CSS, JS
- **Agentic Content Pipeline** · Multi-agent orchestrator with docx build → validate → changelog loop · Claude Code, docx-js, openpyxl

</details>

---

## 🛠️ Tech Stack

**Infrastructure & Ops**

![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=flat-square&logo=ubuntu&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Cloudflare](https://img.shields.io/badge/Cloudflare_Zero_Trust-F38020?style=flat-square&logo=cloudflare&logoColor=white)
![Tailscale](https://img.shields.io/badge/Tailscale-000000?style=flat-square&logo=tailscale&logoColor=white)
![Cisco](https://img.shields.io/badge/Cisco-1BA0D7?style=flat-square&logo=cisco&logoColor=white)
![UniFi](https://img.shields.io/badge/UniFi-0559C9?style=flat-square&logo=ubiquiti&logoColor=white)
![Microsoft 365](https://img.shields.io/badge/Microsoft_365-D83B01?style=flat-square&logo=microsoft&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white)
![C#](https://img.shields.io/badge/C%23-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)

**Frameworks & Platforms**

![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Expo](https://img.shields.io/badge/Expo-000020?style=flat-square&logo=expo&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)
![Jetpack Compose](https://img.shields.io/badge/Jetpack_Compose-4285F4?style=flat-square&logo=jetpackcompose&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)

**AI & Automation**

![Claude](https://img.shields.io/badge/Claude_Code-D97757?style=flat-square&logo=anthropic&logoColor=white)
![MCP](https://img.shields.io/badge/MCP-000000?style=flat-square&logo=modelcontextprotocol&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat-square&logo=ollama&logoColor=white)
![Whisper](https://img.shields.io/badge/faster--whisper-412991?style=flat-square&logo=openai&logoColor=white)
![AssemblyAI](https://img.shields.io/badge/AssemblyAI-2545D3?style=flat-square)
![RevenueCat](https://img.shields.io/badge/RevenueCat-F25A5A?style=flat-square)

**Tools**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Restic](https://img.shields.io/badge/Restic-4A90E2?style=flat-square)
![Adobe Photoshop](https://img.shields.io/badge/Photoshop-31A8FF?style=flat-square&logo=adobephotoshop&logoColor=white)
![Adobe Audition](https://img.shields.io/badge/Audition-9999FF?style=flat-square&logo=adobeaudition&logoColor=white)

---

## 🎓 Certifications

- **Google Cybersecurity Professional Certificate** · Google Career Certificates · 2026
- **Google Project Management Professional Certificate** · Google Career Certificates · 2026

---

## 📊 GitHub Stats

<div align="center">

<img height="200" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=AbhishekAEDan&theme=github_dark" alt="GitHub stats" />
<img height="200" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=AbhishekAEDan&theme=github_dark" alt="Top languages" />

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=AbhishekAEDan&theme=github_dark" alt="Profile details" />

<img src="https://github-readme-streak-stats.herokuapp.com?user=AbhishekAEDan&theme=github-dark-blue&hide_border=true&background=0d1117&ring=00E5A0&fire=00E5A0&currStreakLabel=00E5A0" alt="GitHub streak" />

</div>

---

## 🎯 Beyond Code

`🎮 Gamer` · `🎸 Musician` · `📸 Photographer` · `🎥 Digital Creator` · `☕ Infinite coffee / week`

<div align="center">

**Let's build something solid.**

[![LinkedIn](https://img.shields.io/badge/Connect_on_LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abhishekdan1/)
[![Email](https://img.shields.io/badge/Send_an_Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:danabhishek.work@gmail.com)

<img src="https://komarev.com/ghpvc/?username=AbhishekAEDan&color=00E5A0&style=flat-square&label=Profile+views" alt="Profile views" />

</div>
